...
fn read_words<P: AsRef<Path>>(src: P, n: usize) -> io::Result<HashSet<String>> {
...
fn main() -> Result<(), Box<dyn Error>> {
    let matches = clap::App::new("wordle")
        .arg(
            clap::Arg::new("words")
                .long("words")
                .takes_value(true)
        ...
        .arg(
            clap::Arg::new("filters")
                ...
                .help("the filters to be applied"),
        )
        .get_matches();

    let words = read_words(matches.value_of("words").unwrap(), 5)?;
